...
import TapestryLink from "../TapestryLink.vue"
...
export default {
  ...
  computed: {
    ...mapState(["selection", "settings", "visibleNodes", "links", "nodes", "rootId", "highlightedLinks", "highlightedNodes"]),
    ...mapGetters(["getNode", "getDirectChildren", "isVisible", "getParent", "getNeighbours", "getNeighbouringLinks"]),
    showHalo() {
      // Add code to determine if the node should be highlighted
      return this.highlightedNodes.includes(this.node.id)
    },
    canReview() {
      if (!this.isLoggedIn) {
        return false
  ...
  watch: {
    radius(newRadius) {
      d3.select(this.$refs.circle)
        ...
        .attr("r", newRadius*1.05)
    },
  },
  mounted() {
    this.$emit("mounted")
    this.$refs.circle.setAttribute("r", this.radius)
    this.$refs.halo.setAttribute("r", this.radius*1.05)
  ...
  methods: {
    ...
    ...mapMutations(["select", "unselect", "clearSelection", "setHighlightedLinks", "setHighlightedNodes"]),
    updateRootNode() {
      if (!this.root) {
        this.$router.push({
          name: names.APP,
    ...
    handleClick(evt) {
      if (
        ...
        if (this.root && this.node.hideMedia) {
          this.openNode(this.node.id) 
        } else {
          this.updateRootNode() // root means currently selected
          // Custom code can be put here (or in update Root Node function)
          this.setHighlightedLinks(this.getNeighbouringLinks(this.node.id))
          this.setHighlightedNodes([...this.getNeighbours(this.node.id), this.node.id])
          console.log(this.getNeighbours(this.node.id))
          console.log(this.getNeighbouringLinks(this.node.id))
        }
      } else {
          //Add code here for when you click outside a node you unselect everything
      }
